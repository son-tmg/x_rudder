...
class Heuristic:
    ...
    def minimax(game, currentMove, depth, maximizingPlayer):
        if depth == 0:
        ...
        if maximizingPlayer:
            for y in range(currentMove[0] - 2, currentMove[0] + 3):
                for x in range(currentMove[1] - 2, currentMove[1] + 3):
                    if game.getGameGrid()[y][x] is None:
                        ...
                        childScore = Heuristic.minimax(game, nextMove, depth - 1, False)
            ...
            miniGameGridPossibilities = 0
            for child in miniGameGridPossibilities:
                childScore = Heuristic.minimax(game, child, depth - 1, True)
                if childScore < worst_childScore:
                    worst_childScore = childScore
                    for grid in worst_gridList:
                        worst_gridList.pop()
                    worst_gridList.append(child)
                elif childScore == worst_childScore:
                    worst_gridList.append(child)
    ...
    def searchScore11(game, searchPosition):
        """
        Method to get the score of each individual search positions
        ...
        score3 = 0
        score4 = 0
        score5 = 0.4 * Heuristic.getNumberOfWinningConfigurations11(searchPosition, game)
    ...
    def getNumberOfWinningConfigurations11(searchPosition, game):
        """
            Returns the number of winning configurations for the searchPosition.
            This is determined by determining how many of the 5 winning possible winning configurations can be generated (Center, TL, TR, BL, BR)
            if the token was placed at that position
